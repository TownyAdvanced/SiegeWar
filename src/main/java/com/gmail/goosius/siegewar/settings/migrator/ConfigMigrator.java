...
/**
 * An object which manages the process of migrating config versions to 
 * up-to-date ones.
 */
public class ConfigMigrator {
	...
	public void migrate() {
		...
		for (Migration migration : readMigrator()) {
			...
			if (configVersion.compareTo(migration.version) < 0) {
				...
				SiegeWar.getSiegeWar().getLogger().info("Config: " + migration.version + " applying " + changeCount + " automatic update" + (changeCount == 1 ? "" : "s") + " ...");
		...
		switch (change.type) {
			...
			case REPLACE:
				...
				if (value instanceof String)
					config.set(change.path, ((String) value).replaceAll(change.key, change.value));
				break;
			case MOVE:
	...
	private List<Migration> readMigrator() {
		try (Reader reader = new InputStreamReader(SiegeWar.getSiegeWar().getResource(migrationFilename))) {
