...
public enum ConfigNodes {
	...
	WAR_SIEGE_NATION_SIEGE_EFFECTS_ENABLED(
			"war.siege.switches.nation_siege_effects_enabled",
			"true",
			"",
			"# If this setting is true, then when one nation town is attacked,",
			"# siege effects apply to all nation towns, and the nation itself.",
			"# ",
			"# Purpose:",
			"# Since nation residents generally play as a team on the nation level,",
			"# then for moderate to large nations, town immunity is not sufficient,",
			"# to prevent the team being forced into fighting defensively every day,",
			"# which is generally exhausting for players.",
			"# ",
			"# Effects:",
			"# When an unoccupied home town, or occupied foreign town is attacked,",
			"# The following effects apply:",
			"# All Nation Towns:",
			"#   - PVP forced on.",
    		"#   - Cannot claim.",
			"#   - Cannot unclaim.",
			"#   - Cannot recruit residents.",
    		"#   - Only residents can spawn.",
			"#   + No new siege attacks on the nation.",
			"#   + If a resident dies in town, they keep-env just like soldiers in siege zones.",
			"#   + When the siege ends, immunity is received by all towns",
			"# The Nation Itself:",
			"#   - Cannot recruit towns."),
