...
public enum ConfigNodes {
	...
	HOME_DEFENCE_SIEGE_EFFECTS_ENABLED(
			...
			"true",
			"",
			"# If this setting is true, then home defence siege effects are enabled.",
			"# Summary: ",
			"#   * If a nation home town is attacked, then some mid & post-siege effects are received by the entire home nation.",
			"# ",
			"# Benefits to Players:",
			"#   * A nation cannot be forced to fight more than 1 siege at a time.",
			"#   * After each home defence siege, the entire nation team gets a multi-day break from sieging (default 4.5 days for a full 3 day siege).",
			"#   * Aggressive nations can ignore their own benefits if they wish, by starting new siege attacks on foreign towns,",
			"#      occupying foreign towns which might revolt, or having home towns revolt (revolting bypasses siege immunity).",
			"# ",
			"# Costs to Players:",
			"#   * During a siege, all nation home towns receive some (but not all) of the usual negative siege effects.",
			"# ",
			"# Benefits to Server:",
			"#   * The server better protects player health, as players are no longer forced to fight 24/7.",
			"#   * The server can more carefully control (via the modifier number), the exact amount of siege aggression which they would like on the server.",
			"# ",
			"# LIST OF EFFECTS: ",
			"# HOME NATION: ",
			"# - Cannot add new towns",
			"# NON-BESIEGED HOME NATION TOWNS: ",
      		"# - Cannot claim.",
      		"# - Cannot unclaim.",
      		"# - Residents cannot join.",
			"# - PVP forced on (peaceful towns ignore this effect).",
			"# + Town is immune to siege attack.",
