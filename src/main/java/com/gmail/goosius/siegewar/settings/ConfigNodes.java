...
public enum ConfigNodes {
	...
	ALL_NATION_SIEGES(
		"all_nation_sieges",
			...
			"#######################################################################",
			""),
	ALL_NATION_SIEGES_ENABLED(
			"all_nation_sieges.enabled",
			"true",
			"",
			"# If this setting is true, then all-nation-sieges are enabled.",
			"# ",
			"# Summary: ",
			"#   * When a nation home town is attacked, the nation fights as one team:",
			...
			"#   * During a home-defence siege, all home towns receive a mild version of the usual siege restrictions.",
			"# ",
			"# Benefits to Server:",
			"#   * The server can better protect player health, as players are no longer forced to fight 24/7.",
			"#   * The server can carefully control (via the below 'siege_immunity_modifier' setting), the exact amount of siege aggression which they would like on the server.",
			"# ",
			"# LIST OF WAR RESTRICTIONS: ",
			"# - (Nation) Cannot add new towns",
			"# - (Town) Cannot claim/unclaim.",
			"# - (Town) Cannot add new residents.",
			"# - (Town) PVP forced on (except for peaceful towns, where PVP remains off)"),
	ALL_NATION_SIEGES_SIEGE_IMMUNITY_MODIFIER(
