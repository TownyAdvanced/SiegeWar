...
public enum ConfigNodes {
	...
	POST_WAR_NATION_IMMUNITY_ENABLED(
			...
			"# ",
			"# This feature ensures that nations cannot be forced to fight continuously without a break.",
			"# ",
			"# This effect is achieved as follows:",
			"# 1. The max number of consecutive home-town-defence sieges is limited.",
			"# 2. After each period of continuous fighting, every town in the nation gets siege immunity.",
			"# 3. To mitigate exploits, nations suffer some mild negative effects during sieges - cannot claim/unclaim/recruit.",
			"# 4. To mitigate annoyance-sieges, the cost to attack a nation town grows depending on the size of the defending nation."),
	POST_WAR_NATION_IMMUNITY_MAX_HOMETOWN_DEFENCE_SIEGES(
			"post_war_nation_immunity.max_hometown_defence_sieges",
			"3",
			"",
			"# This setting determines the max number of hometown defence sieges which a nation must fight.",
			"# ",
			"# EXAMPLE:",
			"# If this value is 3, and the nation already has 3 hometowns under attack",
			"# then no more attacks on its hometowns are allowed until after the current sieges end.",
			"# ",
			"# If this value is set to very high (e.g. 999999),",
			"# it allows unlimited consecutive attacks on a nation,",
			"# along with the possibility that the nation could be forced to fight continuously for weeks/months etc."),
	POST_WAR_NATION_IMMUNITY_SIEGE_IMMUNITY_DURATION_MODIFIER(
			"post_war_nation_immunity.siege_immunity_duration_modifier",
			"0.75",
			"",
			"# This setting determines how much siege immunity each nation town receives after each siege/war period,",
			"# ",
			"# EXAMPLE:",
	...
	POST_WAR_NATION_IMMUNITY_HOME_TOWN_CONTRIBUTION_TO_ATTACK_COST(
			...
			"# where small nations cheaply attack larger nations at small towns,",
			"# with no intention of fighting, but just to disrupt the larger nations.",
			"# ",
			"# EXAMPLE:",
