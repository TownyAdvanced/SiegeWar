...
public enum ConfigNodes {
	...
	POST_WAR_NATION_IMMUNITY_ENABLED(
			...
			"# by granting siege-immunity to all its towns after a home-defence war.",
			"# (A home-defence war is when one or more of the nation's towns are under non-revolt sieges)",
			"# ",
			"# Effects on Each Nation:",
			"# 1. The number of simultaneous attacks on the nation's towns is limited (configurable - see next setting).",
			"# 2. After the nation exits a home-defence war, each town gets some siege immunity.",
			"# 3. To mitigate exploits, the nation suffers some mild negative effects while fighting a home-defensive war: cannot claim/unclaim/recruit.",
			"# 4. To mitigate annoyance-sieges, the cost to attack the nation's towns increases as the nation increases in size.",
			"# ",
			"# NOTE 1: ",
			"# This feature only applies to the 'home' towns of each nation (not occupied foreign towns).",
			"# Thus if a nation has chosen to occupy any foreign towns, it is not guaranteed any breaks from war - because this feature does not restrict revolts.",
			"# ",
			"# NOTE 2: ",
			"# If a town switches home nation during a siege",
			"# then when the home-defence war ends, the post-war immunity from that town,",
			"# is distributed between all the home nations which the town had during the course of the siege."),
	POST_WAR_NATION_IMMUNITY_MAX_HOME_DEFENCE_SIEGES(
			"post_war_nation_immunity.max_home_defence_sieges",
			"3",
			"",
			"# This setting determines the max number of home defence sieges which a nation must fight.",
			"# ",
			"# EXAMPLE:",
			"# If this value is 3, and the nation already has 3 hometowns under attack",
			"# then no more attacks on its hometowns are allowed until after all the current sieges end.",
			"# ",
			"# If this value is set to very high (e.g. 999999),",
			"# the restriction is effectively disabled, ",
			"# allowing all hometowns to be attacked at once,",
			"# along with the possibility that nations could be forced to fight continuously i.e. no rest days."),
