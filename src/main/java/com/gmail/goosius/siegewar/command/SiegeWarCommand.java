...
public class SiegeWarCommand implements CommandExecutor, TabCompleter {
	...
	private void showSiegeWarHelp(CommandSender sender) {
		...
		TownyMessaging.sendMessage(sender, ChatTools.formatCommand("Eg", "/sw listunpeacefultowns", "", Translatable.of("sw_command_help_list_unpeaceful_towns").forLocale(sender)));
	...
	private void parseSiegeWarCommand(Player player, String[] args) {
		...
		case "listpeacefultowns":
			...
			break;
		case "listunpeacefultowns":
			parseSiegeWarListUnpeacefulTownsCommand(player);
			break;
		case "town":
	...
	private void parseSiegeWarListUnpeacefulTownsCommand(Player player) {
		TownyMessaging.sendMessage(player, ChatTools.formatTitle(Translatable.of("sw_unpeaceful_towns_title").forLocale(player)));
		for (Town town : TownyAPI.getInstance().getTowns()) {
			if (SiegeWarTownPeacefulnessUtil.isTownPeaceful(town))
				continue;
			TownyMessaging.sendMessage(player, " - <aqua>" + town.getFormattedName());
